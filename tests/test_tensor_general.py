...
import simpletorch
from simpletorch import MathTestVariable, Tensor, TensorBackend, grad_check
...
SimpleBackend = simpletorch.TensorBackend(simpletorch.SimpleOps)
...
if numba.cuda.is_available():
    ...
    shared["cuda"] = simpletorch.TensorBackend(simpletorch.CudaOps)
...
def test_create(backend: str, t1: List[float]) -> None:
    ...
    t2 = simpletorch.tensor(t1, backend=shared[backend])
...
if numba.cuda.is_available():
    ...
    def test_sum_practice() -> None:
        ...
        b = simpletorch.tensor(x)
        s = b.sum()[0]
        b2 = simpletorch.tensor(x, backend=shared["cuda"])
        out = simpletorch.sum_practice(b2)
    ...
    def test_mul_practice1() -> None:
        ...
        z = simpletorch.tensor(x1, backend=shared["fast"]) @ simpletorch.tensor(
            y1, backend=shared["fast"]
        )

        x = simpletorch.tensor(x1, backend=shared["cuda"])
        y = simpletorch.tensor(y1, backend=shared["cuda"])
        z2 = simpletorch.mm_practice(x, y)
        for i in range(2):
            for j in range(2):
...
def test_permute(backend: str, data: DataObject) -> None:
    ...
    simpletorch.grad_check(permute, t1)


def test_mm2() -> None:
    a = simpletorch.rand((2, 3), backend=FastTensorBackend)
    b = simpletorch.rand((3, 4), backend=FastTensorBackend)
    c = a @ b
    ...
    simpletorch.grad_check(lambda a, b: a @ b, a, b)
