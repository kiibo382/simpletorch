...
import simpletorch
from simpletorch import Tensor, TensorBackend, TensorData, UserIndex, UserShape
...
def vals(draw: DrawFn, size: int, number: SearchStrategy[float]) -> Tensor:
    ...
    return simpletorch.tensor(pts)


@composite
def shapes(draw: DrawFn) -> simpletorch.UserShape:
...
def tensor_data(
    ...
    if shape is None:
        shape = draw(shapes())
    size = int(simpletorch.prod(shape))
    ...
    td = simpletorch.TensorData(data, permute_shape)
...
def tensors(
    ...
    backend = simpletorch.SimpleBackend if backend is None else backend
    td = draw(tensor_data(numbers, shape=shape))
    return simpletorch.Tensor(td, backend=backend)
...
def shaped_tensors(
    ...
    for i in range(n):
        ...
        values.append(
            simpletorch.Tensor(
                simpletorch.TensorData(data, td.shape, td.strides), backend=backend
            )
        )
    return values
...
def matmul_tensors(
    ...
    for shape in [l1, l2]:
        size = int(simpletorch.prod(shape))
        data = draw(lists(numbers, min_size=size, max_size=size))
        values.append(simpletorch.Tensor(simpletorch.TensorData(data, shape)))
    return values
