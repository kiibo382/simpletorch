...
import simpletorch
from simpletorch import TensorData
...
def test_layout() -> None:
    ...
    data = [0] * 3 * 5
    tensor_data = simpletorch.TensorData(data, (3, 5), (5, 1))
    ...
    tensor_data = simpletorch.TensorData(data, (5, 3), (1, 5))
    assert tensor_data.shape == (5, 3)
    assert not tensor_data.is_contiguous()

    data = [0] * 4 * 2 * 2
    tensor_data = simpletorch.TensorData(data, (4, 2, 2))
...
def test_layout_bad() -> None:
    "Test basis properties of layout and strides"
    data = [0] * 3 * 5
    simpletorch.TensorData(data, (3, 5), (6,))
...
def test_index(tensor_data: TensorData) -> None:
    ...
    with pytest.raises(simpletorch.IndexingError):
        base[0] = -1
...
def test_shape_broadcast() -> None:
    c = simpletorch.shape_broadcast((1,), (5, 5))
    assert c == (5, 5)

    c = simpletorch.shape_broadcast((5, 5), (1,))
    assert c == (5, 5)

    c = simpletorch.shape_broadcast((1, 5, 5), (5, 5))
    assert c == (1, 5, 5)

    c = simpletorch.shape_broadcast((5, 1, 5, 1), (1, 5, 1, 5))
    assert c == (5, 5, 5, 5)

    with pytest.raises(simpletorch.IndexingError):
        c = simpletorch.shape_broadcast((5, 7, 5, 1), (1, 5, 1, 5))
        print(c)

    with pytest.raises(simpletorch.IndexingError):
        c = simpletorch.shape_broadcast((5, 2), (5,))
        print(c)

    c = simpletorch.shape_broadcast((2, 5), (5,))
    assert c == (2, 5)
